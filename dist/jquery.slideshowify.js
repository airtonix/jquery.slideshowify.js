	...
	$.fn.slideshowify = function( /* config */ ) {
		...
		function _revealImg(curImg) {
			...
			if (imgRatio > docRatio) {
				...
				if (_xDir === 1) {
					$img.css({
						"margin-left": marginPixels * -1 + "px"
					}); // move image before slide if sliding to right
					marginAttr = {
						"margin-left": "0px"
					}; // will be sliding to zero
				} else {
					marginAttr = {
						"margin-left": marginPixels + "px"
					};
				}
				...
				if (_yDir === 1) {
					$img.css({
						"margin-top": marginPixels * -1 + "px"
					}); // move image before slide if sliding down
					marginAttr = {
						"margin-top": "0px"
					}; // will be sliding to zero
				} else {
					marginAttr = {
						"margin-top": marginPixels + "px"
					};
				}
				if (_cfg.direction === "alternate") {
					_yDir *= -1; // switch
